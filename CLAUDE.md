...
- **これで Win/Linux が到達可能に**：残タスク→ #17 CI に Windows/Linux ジョブ追加（tauri-action の matrix）／engine serve の bind 失敗時 `process::exit(1)` を Result 化（デスクトップでポート競合時にアプリごと落ちるのを回避）／`serve.rs` の /report が配信する保存HTMLは bash 生成物＝engine 単体運用時は `disksage scan --html` 前提（engine に html 書き出しを持たせるのは将来）。マルチプラットフォームの本丸②は「動く形」に到達
