...
- **Win/Linux 解禁 ✅**：3OS CIマトリクス追加（上記 desktop.yml）＋**トレイに `.icon()` 追加**（`app.default_window_icon()` 流用）＝Win/Linux でトレイが可視化（`set_title` 非対応OSでもアイコンで見える）。**「ビルドできる ≠ 全機能」**の残タスク（増分）：①Windows のゴミ箱（`to_trash` に Recycle Bin 経路・現状 Win は Err）②engine serve bind 失敗の `process::exit(1)` を Result 化（ポート競合でアプリ落ち回避）③engine が自前で HTML レポート書き出し（/report が bash 生成物依存を脱却）④Linux 実行時 `gio`/`trash-cli` 前提の明記。**本丸②＝マルチプラットフォームは "3OSビルド＋起動" の形に到達**（機能パリティは上記④点を増分で）
