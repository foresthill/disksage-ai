...
- `desktop/` — **Tauri v2 デスクトップアプリ ✅（案A: engine の serve UI をネイティブ窓＋メニューバーで表示）**。`create-tauri-app`(vanilla) 生成→改変。**bash 非依存化 ✅**：`src-tauri/src/lib.rs` は起動時に **`disksage_engine::serve::run(8765)` をスレッドで in-process 起動**（旧: bash `disksage serve` を spawn＝廃止）。tray も engine を in-process 呼び出し＝**アプリ経路から bash が完全消滅**。`src/index.html` は `127.0.0.1:8765` を polling→到達で遷移。将来はネイティブUI(案B/v0.4)へ。`target/`/`node_modules` は gitignore 済
- **3OS リリースCI ✅（`.github/workflows/desktop.yml`・matrix）**：tag `v*` push で Tauri app を **macOS(universal .dmg/.app) / Linux(.deb/.AppImage) / Windows(.msi/.exe)** をビルド→同一 draft Release に添付。手動 dispatch はビルドのみ＝各OSのbundleをartifact化。`tauri-apps/tauri-action@v1`＋`checkout@v7`/`setup-bun@v2`/`rust-cache@v2`/`dtolnay/rust-toolchain@stable`。`fail-fast:false`。Linux は Tauri v2 前提を apt 導入（`libwebkit2gtk-4.1-dev`＋build-essential/curl/wget/file/libxdo-dev/libssl-dev/libayatana-appindicator3-dev/librsvg2-dev＝公式確認済み）。macOS のみ両arch target。旧 `desktop-macos.yml` は本ファイルに統合（削除）。desktop が bash 非依存化したため 3OS ビルドが解禁。**初回CIで各OSビルドを検証**
...
- **本丸② desktop 切替 ✅（実装・CI検証）**：`lib.rs` の bash spawn（`start_serve`/`resolve_disksage`/`ServeProcess`）を全廃し、`std::thread::spawn(|| disksage_engine::serve::run(8765))` に置換。`.build().run(|_,_|{})`（子プロセス無し＝exit kill 不要）。**アプリは単一 Rust プロセス**（serve も tray も in-process）＝**bash 依存ゼロ**。desktop 変更は Tauri ビルド要のため CI で検証（ローカル2GB回避）
- **Win/Linux 解禁 ✅**：3OS CIマトリクス追加（上記 desktop.yml）＋**トレイに `.icon()` 追加**（`app.default_window_icon()` 流用）＝Win/Linux でトレイが可視化（`set_title` 非対応OSでもアイコンで見える）。**「ビルドできる ≠ 全機能」**の残タスク（増分）：**①Windows のゴミ箱 ✅**（`to_trash` に `cfg(windows)` 追加＝PowerShell `Microsoft.VisualBasic.FileIO.FileSystem::DeleteFile/DeleteDirectory(…,'SendToRecycleBin')` で Recycle Bin へ・ファイル/ディレクトリで method 分岐・single quote は `''` エスケープ。mac=osascript/Linux=gio・trash-cli と揃い3OS対応。**Windows実機のruntime未検証・コンパイルはCIのwindowsジョブで確認**）**②bind 失敗の Result 化 ✅**（`serve::run() -> Result<(),String>`、CLI は eprintln+exit(1)、desktop はログのみで継続＝ポート競合でアプリ落ちを回避。実機で同ポート2重起動→Err報告を検証）**③engine 自前 HTML レポート ✅**（`report.rs`＝スキャン完了時に `$DISKSAGE_HOME/scans/<stamp>.html` を書き出し＝bash 無しでも /reports に履歴が溜まる。stamp は `chrono` で `%Y-%m-%d_%H%M%S`（`date` シェルアウトは Win 不可のため crate 採用）。standalone HTML（disk usage＋severity別findings・i18n）。`reports::scans_dir` を pub 化して共用。実機で 空HOME→scan→html生成→/reports 表示→/report 配信 を検証）**④Linux trash 前提を明記 ✅**（engine/README に記載）。**＝本丸②の機能パリティも Win/Linux 向けにほぼ充足**（残: Windows のゴミ箱 runtime 実機確認、engine 側 `scan --html` 相当の CLI 化は任意）
- **v0.1.0 リリース ✅（draft）**：タグ `v0.1.0` push → desktop.yml が3OSビルド→**draft Release に7成果物**（mac universal .dmg/.app.tar.gz・Linux .deb/.AppImage/.rpm・Win .exe/.msi）。publish はユーザー判断（未署名/未notarize＝初回起動警告、Win/Linux runtime 未検証をリリースノートに明記推奨）
- **AI判定の Rust 移植（進行中・Phase 2 の scan --ai 相当）**：**①マスキング ✅**（`mask.rs`＝bash `ai_mask_findings` の忠実移植。SAFE 許可リスト＝既知コンポーネント保持／ユーザー固有dir→`<dirN>`安定エイリアス／数字・swapfileN/sleepimage/docker.raw 保持・`/`・`~` passthrough。**engine初のユニットテスト3件 pass**・clippy クリーン。プライバシー原則＝ファイル内容は一切扱わずパス/メタデータのみ）。**②aリクエスト構築＋レスポンス解析 ✅**（`ai.rs`＝`build_request`＝マスク済み findings＋system＋json_schema(recommendation enum safe_to_delete/archive_then_delete/review_first/keep・confidence high/med/low)で Messages API body 生成、`parse_response`＝error/refusal/content[0].text→judgments を解析。serde/serde_json 追加。**ユニットテスト3件**＝「実ユーザー名は body に出ない（マスク強制）」等を担保・clippy クリーン。純ロジック/ネットワーク無し）。**②b 実HTTP送信 ✅（実装・コンパイル/テスト検証／実送信は未検証）**：`ai.rs` に `resolve_provider`（優先: DISKSAGE_AI_PROVIDER>ANTHROPIC(x-api-key,anthropic-version:2023-06-01,claude-opus-4-8)>OPENROUTER(Bearer,anthropic/claude-opus-4.8)・DISKSAGE_MODEL/BASE_URL override）＋`analyze`（mask→build→ureq POST→parse、HTTP非2xxでも body 読んで API error を parse_response で整形）。`ureq`(rustls) 追加＝12秒ビルド・**ユニット計7件 pass**（provider優先/ヘッダ含む）。**実キー(BYOK)が要るため live 送信は未検証**（キーを平文で受け取らない方針）→ 3OSクロスコンパイルはCI検証。**③CLI 配線＋送信前プレビュー ✅**（`disksage-engine ai [--yes]`＝scan→**マスク済みメタデータのプレビュー表示**（＝実送信物、ファイル内容は送らないと明示）→ `--yes` で `ai::analyze` 送信→判定を index/recommendation/confidence/reasoning で出力。`--yes` 無し=プレビューのみ、キー無し=丁寧にエラー。`lang::init` で reasoning 言語も解決。実機で プレビュー（DeviceSupport→`<dir1>` 匿名化）／no-key 停止を検証）。**＝bash `scan --ai` の中核（mask/request/send/preview/判定）を engine CLI に移植完了**。**serve UI への AI 判定表示 ✅**（`/ai` POST で背景 `analyze`→findings 各行に 🧠 バッジ[推奨=safe_to_delete/archive_then_delete/review_first/keep を色分け・確信度・理由]、opt-inボタン＋「マスク済みのみ送信」注記、判定中バナー＋2秒refresh、API error 整形、no-key 案内。新scanで ai リセット。`Finding` に derive(Clone)、`overview_html` を `page.rs` へ分離し serve.rs 300行維持。実UI検証：no-key→案内／ダミーキー→button→判定中→401整形。成功バッジはコード実装＝実キーで確認）。**監査ログの Rust 移植 ✅**（`audit.rs`＝bash `--ai-log` 相当。`--ai-log` or `DISKSAGE_AI_LOG=1` で `$DISKSAGE_HOME/ai-logs/<stamp>/` に `request.json`（実POST body・マスク済）/`response.json`（生レスポンス＝成功でもHTTPエラーbodyでも）/`masking.tsv`（`real\tmasked\tanonymized`）を保存。`ai::analyze_with_audit` が **送信前に request+masking を書き**（＝ネットワーク失敗しても監査が残る）**送信後に response** を書く。ログ失敗は解析をブロックしない[`let _ =`]。`masking.tsv` は `build_request` と**同じ fresh Aliases・同一順序**で再計算＝モデルが見た内容の忠実な記録。`masking.tsv` は実パス含む＝ローカル限定・非共有。**ユニット2件**[real→masked対応・masked列に実名が出ない]＝計9件 pass・clippy クリーン。**実機検証（ダミーキー・throwaway DISKSAGE_HOME）**：3ファイル生成／request.json にマスク済`<dir1>`・APIキーは body 非混入（ヘッダのみ）／response.json に authentication_error JSON を捕捉。CLIは `ai --ai-log`、serve は `DISKSAGE_AI_LOG=1` で起動）。実送信は BYOK キー必須でユーザー実行
...
- **言語切替を即時反映 ✅（2026-09-25）**：`lang.rs` を `OnceLock<bool>`（起動時固定・再設定不可）→ `AtomicBool` + `refresh()` に。`/settings` の POST で `write_lang` 後 `lang::refresh()` を呼ぶ＝**設定変更が次のリクエストで即反映**（旧: 再起動まで英語のまま＝ユーザーが「日本語にしたのに英語」で面食らう問題）。settings 注記も「再起動不要」に更新。**実サーバ検証**（throwaway HOME・env空）：default=設定(JA・AppleLocale ja_JP)／POST en→Settings 即時／POST ja→設定 即時／POST 空→AppleLocale で JA。clippy クリーン・9テスト pass。**根本原因メモ**：ユーザーが「英語で急に出る」と感じたのは、旧セッションの英語固定 bash serve が `:8765` を占有し desktop の engine serve(=AppleLocale で日本語)が bind できず英語UIにフォールバックしていたため（＝検出バグではなく状態）。engine serve は OS ロケールを正しく日本語に解決する
- **クロスプラットフォーム言語自動判定 ✅（2026-09-25・`sys-locale`）**：`lang.rs` の detect() を macOS 限定の `defaults read AppleLocale`（サブプロセス）→ **`sys-locale` crate**（`get_locale()`＝mac/Win/Linux 共通で "ja-JP" 等を返す）に置換。**動機**：旧実装は AppleLocale しか見ず、**Windows/Linux では自動デフォルトが英語**に落ちていた＝日本語ユーザー（社長さんの Windows 等）が英語で面食らう。precedence は DISKSAGE_LANG→config→LC_ALL/LANG→**OSロケール(sys-locale)**→en。実サーバ検証（env クリア・throwaway HOME）：sys-locale から `設定`(JA) を確認。clippy クリーン・9テスト pass。サブプロセス起動も不要に
- **保存レポートのナビ修正 ✅（2026-09-25）**：`/report?stamp=` が保存済み standalone HTML をそのまま返し**サイドバー消失＋戻る導線なし**だった不具合を修正。`reports::saved_report_body`（`body_inner` で `<body>` 内側を抽出）で本文だけ取り出し、serve の shell（サイドバー付き）に**読み取り専用バナー＋「← スキャンに戻る」**を添えて埋め込む。実サーバ検証：sidebar(position:fixed)／戻るリンク／本文／traversal 404 を確認。ユーザー指摘「レポートを開くとサイドメニューが消えて戻れない」への対応
- **AI トークン使用量の表示 ✅（2026-09-25）**：`ai.rs` に `Usage{input_tokens,output_tokens}`＋`Analysis{judgments,usage}` を追加。`parse_response` が API レスポンス top-level の `usage` を取り込み、`analyze`/`analyze_with_audit` が `Analysis` を返す。serve の AI パネルに **🪙 入力/出力トークン**を表示、CLI `ai` も判定後にトークン要約を出力。**コストの $ は出さない**（モデル別価格は変動＝憶測回避。トークン数は API 由来の事実のみ）。ユニットテストで usage 解析を担保（input10/output5）＝9件 pass・clippy クリーン。live 表示は BYOK 実キー必須のため未検証（他AI送信同様）。ユーザー質問「何を聞いて何が返ったか・トークンいくら」への回答＝監査ログ(--ai-log)＋この使用量表示
- **Quick / Full（じっくり）スキャン ✅（2026-09-25）**：Quick（既定・既知パターンを高速表示）に加え、**Full scan**（POST `/fullscan`）で深掘り2種を追加＝**`top.rs`**（大きいフォルダ＝$HOME直下＋Library各subdir＋隠しキャッシュを**並列 du**、bash `cmd_top` 移植）／**`flow.rs`**（最近増えたファイル＝500MB超・30日以内、node_modules/.git/Caches/.cache を prune、bash `find_flow_type_files` 移植・メディア拡張子は「外付けへ退避」マーク）。`deep.rs`＝`DeepScan{top,flow}`＋`run()`＋`section_html()`（トグルボタン/実行中バナー/結果2セクション）。serve は `ScanState.deep`/`deep_running`＋`/fullscan` を背景スレッドで実行（$HOME全走査のため）・新スキャンで deep もクリア・実行中は2秒 refresh。**engine 未移植だった top/flow をここで移植**＝bash とほぼ parity。**ファイル分割**：serve.rs が363行になったため `sidebar`/`shell`→`page.rs`、AIパネル(`ai_controls`/`ai_button`/`ai_configured`)→新 `ai_ui.rs`（ScanState 依存を外し `(ai, running)` 引数化）に抽出＝**全ファイル300行以下復帰**（serve252/deep130/page103/ai_ui78/flow73/top64）。実サーバ検証（throwaway HOME）：Quickに「じっくり」ボタン→POST→大きいフォルダ/最近増えたファイル出現。clippy クリーン・9テスト pass。ユーザー要望「パッと開きたい人と根掘り葉掘りしたい人」への対応
- **AI プロバイダに Claude CLI 追加 ✅（2026-09-25・BYOKキー不要）**：`claude_cli.rs`＝ユーザーが入れてログイン済みの **Claude Code を `claude -p` でサブプロセス実行**し AI 判定（API キー不要＝サブスク/ログインをそのまま利用）。**Claude Code のトークンを流用するのではなく `claude` を正規起動**する点が肝（規約/セキュリティOK）。**実測で仕様確定**（`claude -p "…" --output-format json --json-schema <inline>` < /dev/null）：返り値の **`structured_output.judgments`** に構造化判定、`usage`（input/output）＋**`total_cost_usd`**。→ `ai::build_prompt`（system+マスク済findings を1プロンプト化、`user_text`/`system_text` を HTTP経路と共有）＋`output_schema` を渡す。`resolve` 優先順位：`DISKSAGE_AI_PROVIDER=claude-cli` 明示 → 他明示provider → 自動（キー未設定 かつ `claude` が存在）。`available()` は spawn せず既知パス（`~/.local/bin/claude` 等・`DISKSAGE_CLAUDE_BIN` 上書き可）を探索＝GUI(.app) の狭い PATH 対策。`Analysis` に `cost_usd` 追加＝CLI 経路のみ **$ を正確表示**（HTTP は価格未知で $ 出さない方針は維持）。監査ログ(--ai-log)も CLI 経路対応（prompt=request / 生JSON=response）。ai_ui は `ai::available()`（key か claude）でボタン出し分け＋トークン/コスト表示。プライバシー不変＝マスク層を通したメタデータのみ。**⚠️ コスト注意**：`claude -p` は Claude Code 自身の巨大コンテキスト（cache_creation ~90k tokens）を毎回積むため **API 直叩きより1回が高い**（実測 $1.01/回）。サブスク運用なら実費ゼロ（使用枠消費）だが API 課金だと高額＝UIで cost 明示。**ユニット2件**（実エンベロープの parse・エラー）＝計11件 pass・clippy/fmt クリーン。**実 e2e（`ai --yes` 経由の統合送信）は追加 ~$1 課金回避のため未再実行**＝実測(1回)＋parse単体＋コードレビューで担保。ユーザー質問「Claude 入れてたら使ってくれる運用が嬉しい」への回答＝この claude-cli プロバイダ
- **集計ドリルダウン＋個別選択削除 ✅（2026-09-25）**：`node_modules_aggregate`/`library_caches` の**集計もの**（丸ごと削除は危険なので従来チェックボックス無し）を、**内訳に展開して個別に選んでゴミ箱へ**できるように。`breakdown.rs`＝`Kind`(NodeModules/LibraryCaches)・`items()`（NodeModules=~/Development 配下の各 node_modules を走査・nested内は二重計上回避／LibraryCaches=~/Library/Caches 直下の各アプリ）サイズ降順 top100・`is_deletable()`=**構造的検証**（スコープ＋形＋存在＝クライアントの任意パスを信用しない）・`page()`（個別チェックボックス＋赤い削除ボタン+JS confirm＋戻る）・`handle_post()`（parse→検証→to_trash→redirect）。findings に「🔍 内訳を見て選ぶ →」リンク（`Kind::for_finding`）。serve に GET/POST `/breakdown`。**実サーバ検証**：projA/projB の node_modules 列挙＋チェックボックス／不正kind→404／**範囲外パス(/etc/hosts)POST→ゴミ箱移動されず（構造検証OK）**／有効パス選択→source から消え未選択projAは残存。**安全性の再確認**：`to_trash` は osascript の Finder move-to-trash（rm しない）。**重要な学び**：`ls ~/.Trash` はシェルに Full Disk Access が無いと **TCC でブロックされ空に見える**（＝消えたのではない）。AppleScript 返り値 `…of item .Trash of folder <user>…` が `~/.Trash` 移動を明示＝復元可を確認。全ファイル300行以下（serve291/breakdown236/findings144）・clippy/fmt クリーン・11テスト pass。ユーザー要望「集計の中身を見て、実際にどのファイルが消えるか選んで確認して削除したい」への対応
- **新パターン3種を JSON だけで追加 ✅（2026-09-26・コード変更ゼロ）**：`patterns/builtin.json` に `photos_library`（~/Pictures/Photos Library.photoslibrary >10GiB・**info**・「二度と戻せない・削除するな・iCloud最適化 or 外付けへ」）／`apple_mail`（~/Library/Mail >5GiB・low・IMAP再同期／添付整理）／`downloads_folder`（~/Downloads >5GiB・low・古いインストーラ整理）を追記。**Rust は一切触らず**＝#67 の外出しの成果を実証。全て report のみ（DELETABLE 非対象＝ワンクリック削除不可・特に写真は info で誤削除防止）。実機 scan で `downloads_folder: 7.2 GB` 発火を確認（photos/mail は閾値未満で非発火＝正しい）。builtin パース単体テスト12件 pass・clippy/fmt クリーン。README 検出表にも追記
- **パターンの JSON 外出し ✅（2026-09-26）**：単純な「ディレクトリ×閾値」系9パターン（ollama_models/user_cache/npm_cache/pnpm_store/library_caches/xcode_derived_data/ios_devicesupport/coresimulator_caches/coresimulator_devices）を **`patterns/builtin.json`** に外出し＝コードのハードコードを廃止。`engine/src/patterns.rs`＝`include_str!("../../patterns/builtin.json")` で同梱＋`$DISKSAGE_HOME/patterns.json` でユーザー/コミュニティ追加（`OnceLock` で1回ロード）。`PatternJson`（id/path|paths/threshold_gib/severity/label_en,ja/action_en,ja）→ `compile()` で `Pattern`（id/severity は **1回だけ intern して `&'static str`**＝`Finding` の型は不変・毎スキャンのリークを回避、label/action は owned・is_ja で選択）。`scan.rs` は9個の `dir_pattern` 呼び出し→`patterns::all()` 汎用ループに置換、`dir_pattern` 削除。**特殊ロジックはコードのまま**（node_modules集計/docker_raw/iphone_backup/apfs_snapshots/vm_swap/electron_cache）。`paths` 配列で pnpm の mac/linux fallback を表現。severity は既知値へマップ（不明→info）。**検証**：builtin.json parse 単体テスト（+1＝12件）／実機 scan で JSON駆動の library_caches・ios_devicesupport が発火＝従来と同一 id／**ユーザー patterns.json 追加が再ビルド無しで反映**（custom_mytool 実証）。`patterns/README.md` にスキーマ＋CC-BY-SA 明記（コード=Apache-2.0／パターン=CC-BY-SA の方針に合致）。README の「declarative JSON」記述を実態に更新。clippy/fmt クリーン。ユーザー要望「パターンを JSON 外出し」への対応
- **engine パターン parity 完成 ✅（2026-09-26）**：bash にあって engine に未移植だった **`coresimulator_devices`**（~/Library/Developer/CoreSimulator/Devices >5GB・medium・DELETABLE 済＝削除でシミュレータ状態消失）と **`electron_cache`**（~/Library/Application Support/<app> の Cache/Code Cache/GPUCache 合計 >500MB・アプリ別 finding・safe・path=アプリ本体なので DELETABLE 非対象＝report のみ）を engine `scan.rs` に移植。`check_electron_cache`＝appsup 直下を走査し3キャッシュ合計。実機で Claude 1.1GB / obsidian 786MB を検出＝bash と一致。**＝engine の finding パターンが bash と完全 parity**（残る flow_type は engine では Full scan で対応済）。clippy/fmt クリーン・11テスト
- **AI キーの OS キーチェーン保存 ✅（2026-09-26・`keyring`）**：設定画面に **AI（BYOK）キー欄**を追加＝プロバイダ選択（Anthropic/OpenRouter）＋パスワード入力→**OS キーチェーン**（mac Keychain / Win Credential Manager / Linux Secret Service）に保存。`secret.rs`＝`keyring` crate ラッパ（service=`disksage`・account=env名 `ANTHROPIC_API_KEY`/`OPENROUTER_API_KEY`・get/has/set/delete）。Cargo `keyring = { version="3", features=["apple-native","windows-native","sync-secret-service","crypto-rust"] }`（crypto-rust で Linux も pure-Rust＝openssl/dbus 不要）。`ai::resolved_key` で **env → キーチェーン** の順に解決（`resolve_provider` と `use_claude_cli` の「キー有無」判定の両方に反映）。`settings::handle_post` が lang フォームと AI フォームを判別（AI キーは keychain のみ・**config には絶対書かない**＝設計原則「平文保存禁止」）。キー値は画面に一切表示せず「保存済み(プロバイダ名)」＋「保存キーを削除」だけ。**GUI(.app) からも BYOK キー設定可**＝Finder 起動で env が渡らない問題も解消（claude-cli と合わせAI が使える3経路：env / keychain / claude CLI）。**実機往復検証**：未保存表示→ダミー保存(303)→「キーチェーンに保存済み」読み戻し→**config に平文キー無し**→クリアで keychain 削除、を確認（ダイアログ無し）。keychain はグローバルで DISKSAGE_HOME 分離不可＝テストは security コマンドで後始末。ユニットテストは付けない（keychain バックエンドが要る＝CI headless で不可）。全ファイル300行以下・clippy/fmt クリーン・11テスト。ユーザー要望「BYOK キーを安全に設定したい」への対応
- **ドリルダウンに `~/.cache` 追加 ✅（2026-09-25）**：`Kind::UserCache`（`user_cache` finding）を追加＝`~/.cache` 直下の各開発ツールキャッシュ（uv/huggingface 等）を個別選択削除。cache系2種を `container_for()`＋`list_subdirs()` に共通化。findings リンクは `for_finding` 経由で自動対応（findings.rs 変更不要）。実サーバ検証：uv/huggingface 列挙＋範囲外パス拒否。clippy/fmt クリーン・11テスト
- **v0.1.1 公開 ✅（pre-release・2026-09-25）**：engine CI/#45・監査ログ/#44・npm/pnpm パターン/#46・トレイ恒久化/#47 を含む初の**世界公開**リリース。タグ `v0.1.1`→desktop.yml で3OSビルド成功→7成果物（mac .dmg/.app・Linux .deb/.AppImage/.rpm・**Win .exe/.msi**）。未署名・Win/Linux runtime 未検証のため **prerelease** ラベル＋リリースノートに SmartScreen/Gatekeeper 回避手順を明記。社長さんの Windows 実機テストが初の runtime 検証
