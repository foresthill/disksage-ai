...
pub fn ai_controls(ai: Option<&Result<Analysis, String>>, ai_running: bool) -> String {
    ...
    // A completed analysis: show the token usage (factual) + cost when known + re-run.
    if let Some(Ok(a)) = ai {
        let usage = a
            .usage
            .as_ref()
    ...
    if !ai::available() {
        return format!(
            ...
            t(
                "AI judgment needs either a BYOK key (ANTHROPIC_API_KEY / OPENROUTER_API_KEY) or Claude Code (the `claude` CLI) installed and logged in.",
                "AI判定には BYOK キー（ANTHROPIC_API_KEY / OPENROUTER_API_KEY）か、Claude Code（`claude` コマンド）のインストール＋ログインが必要です。",
            )
        );
    }
    ai_button(false)
